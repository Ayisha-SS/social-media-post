...
function MyPosts() {
  ...
  return (
    <div>
      ...
      <div className='py-4 fixed top-0 left-0 right-0 border-b-4 border-b-solid border-b-purple-500 shadow-md bg-slate-200'>
        <div className='wrapper flex justify-between'>
          <h1>My Posts</h1>
          <Link to="/create" className="bg-gradient-to-r from-purple-600 to-pink-500 text-white py-2 px-4 rounded">
            Create Post
          </Link>
        </div>
      </div>
          ...
          posts.map(post => (
            <div key={post.id} className='flex flex-col w-full items-start border-b-2 pt-2 pb-4 h-[500px] max-[768px]:h-[400px]'>
              <div className='flex items-center gap-5'>
                ...
                <span>
                  <h3 className='text-2xl font-medium max-[768px]:text-[20px] cursor-pointer'>{post.created_by}</h3>
